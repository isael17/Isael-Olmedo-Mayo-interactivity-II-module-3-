function spawnFlyingStuff(event) {
    ...
    for (let i = 0; i < numItems; i++) {
        ...
        const randomY = (Math.random() - 0.5) * 500; // Fly up or down
    ...
    const messages = ["Anime IS The BEst In The World!", "Keep Ckicling For More Anime Info!", "Aniem IS very Nice And Cool!", "Anime Waifus Are the Worst!", "Demon Slayer Is The Best"];
    const colors = ["#e74c3c", "#3498db", "#2ecc71", "#f1c40f", "#9b59b6"];

    function createButton() {
        const btn = document.createElement("button");
        btn.className = "falling-btn";
        btn.innerText = "Why Don't You Click Me";
        ...
        btn.style.animationDuration = (Math.random() * 3 + 2) + "s"; // 2-5 seconds
        ...
        // Clean up memory: remove button after it falls off screen
        btn.addEventListener("animationend", () => btn.remove());

        document.body.appendChild(btn);
    }

    // Spawn a new button every 1 second
